...
const { persistDownload } = require('./persist-download.cjs');
...
async function main() {
  ...
  const waitForDownloads = async () => {
    const pending = Promise.allSettled([...pendingDownloads]);
    await Promise.race([
      pending,
  ...
  try {
    ...
    const watchPage = page => {
      ...
      page.on('download', download => {
        const saving = persistDownload(download, profileDir).catch(() => {
          process.stderr.write('Could not save a Chrome download to the persistent home.\n');
        });
        pendingDownloads.add(saving);
        void saving.finally(() => pendingDownloads.delete(saving));
      });
    };
