...
- When login or another human-only step is required, call
  ...
  to either one.
- The permanent link returned by `get_novnc_link` is tailnet-only and is safe
  to bookmark in the user's password manager. It remains stable until the
  server operator rotates gateway credentials.
- Only when the user explicitly says they cannot use Tailscale, call
  `create_temporary_novnc_link`. It accepts no arguments, temporarily enables a
  guest-only public Funnel endpoint, and returns both the permanent tailnet link
  and a single-use public link with a fixed 30-minute maximum lifetime.
- When the user finishes on a guest machine, call
  `revoke_temporary_novnc_link`. Do not create temporary public access merely
  because login is required or because the permanent link is inconvenient.
