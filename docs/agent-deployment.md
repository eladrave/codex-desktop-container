...
9. **Timezone and desktop sizes:** Confirm the timezone and desired CRD sizes.
...
13. **Chrome Remote Desktop:** Ask the user to choose the Google account in their
    own browser, generate the short-lived Linux command, paste it directly into
    the trusted Tailscale SSH session, and enter the PIN at its hidden prompt.
...
Tailscale and the CRD desktop session.
...
The agent must not ask the user to paste the CRD authorization code or PIN into
chat. Direct the user to:
...
- CRD and noVNC show the same Xfce session.
- Codex and Chrome are running as UID 10001.
- Chrome reports connected in Codex.
- One harmless `@Chrome` task succeeds.
- Container recreation preserves Tailscale, CRD, Codex, extension, browser
  state, and noVNC configuration.
